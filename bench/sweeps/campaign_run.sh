...
#
# with no argument it runs every configuration in the table. with names it runs only those, which is how a single cell is repeated without touching the rest. a configuration whose capture directory already holds a non-empty telemetry.bin is skipped, so an interrupted campaign continues where it stopped rather than starting over.
#
...
# the UART carries framed binary telemetry records interleaved with the human readable status lines, so a line matched out of that stream can hold bytes that are not text. macOS tr answers "Illegal byte sequence" and dies on a byte that is not valid in the current locale, and the tr calls that take a confirmed status line apart run on exactly such a line. the grep that produced it already carries -a and LC_ALL=C for the same reason, so the locale belongs to the script rather than to individual pipelines.
export LC_ALL=C
...
# the stack region is fixed when the thread is created, so a change to it resets the board and the rest of the configuration has to wait for the board to come back. bytes that arrive during boot are dropped, which is how the first attempt at this lost its victim byte.
...
run_one() {
  ...
  # an optional extra console byte, for a configuration the six main knobs cannot express. "-" is the normal case and sends nothing.
  local extra="${9:--}"
  local stackb="${10:-j}" arenab="${11:-7}"
  local dir words passes want status

  # a capture whose stress.txt carries a void line does not count as done, so a configuration that produced an unusable capture is taken again under a new timestamp. the unusable one stays on disk with its reason beside it, since results/raw is append only.
  ...
  # the stack request goes first and on its own. a region that is already in use resets nothing, so a run that keeps the same stack across rows pays the reboot once rather than per capture.
  if [ "$stackb" != "$LAST_STACK" ]; then
    send_bytes "$stackb"
    sleep "$REBOOT"
    LAST_STACK="$stackb"
  fi
  # the extra column is a string of console bytes for anything the six main knobs cannot express. "-" sends the two bytes that put the board back in its default state.
  if [ "$extra" = "-" ]; then
  ...
  # the golden vector is the only end to end check this firmware has, and a stack page that landed somewhere wrong would break the setup rather than the measurement. a capture is not taken against a board that is no longer classifying the known window correctly.
  if [ "$vic" = i ] || [ "$vic" = v ]; then
  ...
  # the sweep, the victim and the footprint are not in the record. the wire format was not changed for this campaign, so they go beside the capture, and the board's own status line goes with them so the filing can be checked against what the board said rather than against this table.
  {
    printf 'campaign=%s\n' "$CAMPAIGN"
    printf 'experiment=%s\n' "$experiment"
    printf 'victim=%s\n' "$(victim_name "$vic")"
    ...
    # the image every capture of this campaign has to share, copied from the capture's own build.txt rather than recomputed, and the configuration label the comparison is filed under.
    ...
    # what the victim's address stream actually does, counted from the disassembly of the image that ran rather than taken from the knob. only the read loop has one.
    if [ "$vic" = r ]; then
      ...
      # inference has a data dependent call graph, so the static count the read loop gets is not available. what is known from the code is which regions it touches at all.
