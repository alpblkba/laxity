...
#
# written but never executed. it was validated by reading campaign_run.sh and tools/stm32/lib.sh and by bash -n, and by nothing else. no cell of it has been run against a board, so every timing constant below is inherited from campaign_run.sh rather than measured here.
#
# with no argument it prints usage and does nothing, since every arm here is a long board session and none of them should start by accident.
#
# what this campaign measures is where the measurement thread's stack lands when it comes from the ThreadX byte pool instead of from one of the three fixed pages, and what a block taken out of that pool ahead of it does to the answer. the console bytes for both are new: p selects the vendor default stack, and f, g, h, m and q select a ballast of 0, 64, 256, 1024 and 4096 bytes. each one resets the board, because a thread's stack is fixed when the thread is created and the ballast has to be taken before that.
#
# the health arm exists for a bench condition rather than for the campaign. the board's JP3 IDD jumper was bent and the target read 0.00 V for a day, so the first thing wanted after the repair is two identical captures under DMA load, which is where an intermittent supply shows up and idle does not. its two numbers belong to this image alone and comparing them against a capture taken with any earlier image compares two binaries and not two board states.
set -euo pipefail
cd "$(dirname "$0")/../.."
. tools/stm32/lib.sh

# the UART carries framed binary telemetry records interleaved with the human readable status lines, so a line matched out of that stream can hold bytes that are not text. macOS tr answers "Illegal byte sequence" and dies on a byte that is not valid in the current locale, which is what killed the first health run: the grep ahead of it had been told the stream was bytes and the tr after it had not, so it died, await_line answered empty, and the caller read that as the line never having arrived.
#
# every filter in this script reads that same stream, so the locale belongs to the script rather than to individual pipelines. the explicit LC_ALL=C prefixes further down are redundant once this is set and they stay, because they carry the reason at the point where it applies.
export LC_ALL=C

SECS="${LAXITY_SECS:-75}"
# long enough for the board to finish the pass it was in the middle of, which is the same 8 seconds campaign_run.sh uses for the same reason.
SETTLE="${LAXITY_SETTLE:-8}"
CONFIRM="${LAXITY_CONFIRM:-20}"
# this script reboots the board 80 times in arms a and b alone, so REBOOT is the largest single term in its wall clock rather than a detail: at the default it is 16 minutes of the run. the read that follows every reboot waits up to CONFIRM seconds for the board's line anyway, so lowering REBOOT trades wall clock against how much of the boot the serial read sits through, and it cannot be lowered past the point where the reset itself has not taken effect.
REBOOT="${LAXITY_REBOOT:-12}"
# the virtual COM port keeps a backlog while nothing is reading and the backlog survives closing and reopening the port, measured in capture.sh. a boot line read without draining first can therefore be the previous boot's line, which would be recorded as this boot's address and would be wrong in the one way this campaign cannot detect afterwards. what the drain throws away is a mix of stale and fresh lines rather than stale ones only, since laxity_export_entry prints the whole human readable set once per second and not once at boot, app_threadx.c around line 1591. discarding a fresh line costs nothing because the next second carries the same one, and that repetition is the whole reason a blind drain is safe here.
BOOTDRAIN="${LAXITY_BOOTDRAIN:-3}"

# fifteen boots per half of arm a, thirty in total, and the halves bracket arm b so that arm a is its own control against anything that moved during arm b.
BOOTS_A=15
# ten boots per ballast value, fifty in total.
BOOTS_B=10

# one directory for the whole campaign rather than one per cell, since the boot tables are not captures and nothing in results/raw's timestamped layout reads them. it sits under results/raw because it is append only experiment data and because that path is already in .gitignore.
CAMPAIGN_DIR="${LAXITY_CAMPAIGN_DIR:-results/raw/placement-determinism}"
CAMPAIGN=placement-determinism

# the ballast sweep, as console byte and byte count, in the order arm b runs them. the counts mirror laxity_ballast_sizes in the firmware and the board prints what it actually allocated, which is what the table records.
BALLASTS="f:0 g:64 h:256 m:1024 q:4096"

# the console byte for the vendor default stack, which is what arms a and b both measure. 15 is LAXITY_STACK_SRC_POOL as the board reports it.
POOL_BYTE=p
POOL_SRC=15
...
# one read window, answered as the infer line and then the placement line. both come out of the same window because the pass counter is a witness for the address recorded beside it, and one read later it would be a witness for a different second of the same boot. the exporter prints the set once per second in a fixed order with the infer line ahead of the placement line, so awk holds the first and prints the pair when the second arrives, which ends the read the way grep -m1 does.
await_boot_lines() {
  exec 3<>"$PORT"
  stty -f "$PORT" 921600 raw -echo
    # the records interleave with the text and carry NUL bytes, and macOS awk holds a record as a C string, so it stops at the first one and never reaches the match. grep is binary safe, which is why await_line does not need this and this does.
...
# one space separated key=value out of a line the board printed, by exact key, so that ballast does not also answer for ballast_addr. awk reads the whole stream rather than stopping at the first match, since a stage that exits early leaves the one above it writing into a closed pipe and pipefail turns that into a failed pipeline. it is the fault lib.sh describes with grep -q.
field() {
  printf '%s' "$2" | tr ' ' '\n' \
...
# put the board on a stack source and a ballast size, then read back what it says it is on. both bytes reset the board when they change something and do nothing at all when they do not, so each one is followed by a reboot wait and the configuration is confirmed from the board's own line rather than from the request.
select_config() {
...
boot_once() {
  # a hardware reset through the probe rather than a console byte, since the console bytes only reset when they change something and this loop asks for the same configuration at every boot.
  #
  # its status is not swallowed. a reset that did not happen leaves the board running, and arm a would then record fifteen identical addresses and report a determinism it never tested, which is a false negative on the question the campaign exists to ask. nothing in the table afterwards separates that from fifteen real boots that agreed.
  STM32_Programmer_CLI -c port=SWD mode=Hotplug -rst >/dev/null 2>&1 || return 1
  exec 3<>"$PORT"
  stty -f "$PORT" 921600 raw -echo
  # timeout answers 124 when it reaches its limit, which is what a drain that ran for its whole window looks like, so this one is absorbed.
  "$TIMEOUT" "$BOOTDRAIN" cat "$PORT" >/dev/null 2>&1 || true
  exec 3>&-
  await_boot_lines
}

# n boots into one tab separated table. the table is written to a partial file and moved into place at the end, so an interrupted cell leaves nothing that the skip rule would read as a finished one, and the partial stays on disk.
boot_loop() {
  ...
  while [ "$i" -le "$n" ]; do
    ...
    # the reboot witness. it is the pass counter this boot has reached, read out of the same window as the address beside it. a reset that silently did not happen leaves it climbing from boot to boot instead of restarting near zero, and that is the only trace such a boot leaves in the table. it is recorded and not judged: no threshold here and no verdict.
    ...
    # a boot that came back on another source or another ballast is not a boot of this cell, and filing it under this cell would put two configurations in one table. the line is on disk before the stop, since it is the evidence.
...
#
# this arm cannot average through a restart at all. it exists to find a marginal supply after the JP3 repair, and a brownout in the middle of a capture is exactly the shape that failure takes, so a median across one would average the thing being looked for into the thing it is being compared against.
capture_stats() {
  python3 - "$1" <<'PY'
import pathlib
import statistics
...
# one health capture. the console bytes are the ones stress-fn-gate-open-1 used, j i 0 a X C L 7 with N and R putting the two knobs it does not name back in their normal state, and both captures send the same sequence.
health_one() {
  local name="$1" dir status line

  # a capture killed part way through writing leaves a non-empty telemetry.bin and no stress.txt, which is how stress-pr-gate-close-2 was lost, so both have to be there before a cell counts as done.
  ...
  # the ballast survives a reset in the same word the stack source does, so a health cell run after arm b would otherwise carry whatever arm b left in it. it is read rather than set, since the byte sequence for this arm is fixed.
  line="$(await_line 'placement ok=')"
  if [ -z "$line" ]; then
    echo "board printed no placement line within ${CONFIRM}s" >&2
    exit 1
  ...
  # the sweep, the victim and the placement are not in the record. the wire format was not changed for this campaign, so they go beside the capture, and the board's own two lines go with them so the filing can be checked against what the board said rather than against this script.
  {
    printf 'campaign=%s\n' "$CAMPAIGN"
    printf 'arm=health\n'
    ...
    # sampled when the configuration was confirmed and the schedule is shuffled, so this is whichever point was active then and not the point the median below reads. it is kept under a name that says so.
    ...
    # tab and printable ASCII only. what is dropped is the frame bytes that were interleaved ahead of the match, which are an artifact of one UART carrying the records and the text together rather than anything the board said. the records themselves are in telemetry.bin, and these two fields exist to check a capture against the name it was filed under, which the board's own text does on its own.
...
arm_health() {
  ...
  # a capture that spans a board reset carries two configurations, so neither a median of it nor a difference against it is a number about one of them.
  if [ "$r1" != 0 ] || [ "$r2" != 0 ]; then
