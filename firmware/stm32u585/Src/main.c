...
static void MX_SPI2_Init(void)
{
  ...
  /* ST sets this in every one of its Wi-Fi projects for this module and it is in no .ioc.
...
/* USER CODE BEGIN 4 */
/* the U5 HAL splits the EXTI callback into rising and falling. the plain HAL_GPIO_EXTI_Callback
   name compiles and is never called on this part.

   both calls matter and they do different things. mxchip_WIFI_ISR advances the driver's SPI
   state machine, and nx_driver_emw3080_interrupt hands the event to deferred processing. with
   only the first, the link comes up and no packet is ever delivered. */
...
/* CubeMX drops NVIC.EXTI14_IRQn and NVIC.EXTI15_IRQn from the .ioc, so the enable and the two
   handlers are written here. both handler names are weak in the vector table, so a definition
   in this translation unit replaces the default one. */
