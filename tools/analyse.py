...
set LAXITY_RESULTS to write somewhere other than RESULTS.md at the repository root.
    ...
    ./tools/analyse.py [--allow-simulated] results/raw/<run> [more runs...]
"""
...
def entry(run_dir, allow_simulated=False):
    ...
    simulated = meta.get("origin") == "simulated"
    if simulated and not allow_simulated:
        raise SystemExit(
            "%s carries simulated telemetry; pass --allow-simulated to report it" % run_dir
        )
...
def main():
    ...
    entries = [entry(pathlib.Path(run), args.allow_simulated) for run in args.runs]
    body = ["# Results", ""]
    if any(simulated for _, simulated in entries):
        body.extend([
            "Simulated telemetry. This document contains host-generated input accepted with "
            "`--allow-simulated`; it is not evidence from the STM32 board.",
            "",
        ])
    body.extend([
    ...
    body.extend(text for text, _ in entries)
    OUT.write_text("\n".join(body).rstrip() + "\n")
    print("wrote %s" % OUT)
